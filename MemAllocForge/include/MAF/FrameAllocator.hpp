...
namespace MAF
{
    /// @brief N-buffered frame allocator for per-frame scratch memory.
    /// @details Each of the N frames is an independent linear arena. SwapFrames() rotates
    ///          the active frame and resets the recycled one. Deallocate() validates but does
    ///          not free; storage is reclaimed only on frame rotation. Not thread-safe.
    /// @tparam N Number of frame buffers (>= 2, default 2).
    /// @param frameSize Size of each frame in bytes.
    /// @param defaultAlignment Default alignment (must be power of 2).
    /// @throws std::invalid_argument If defaultAlignment is not a power of 2 or frameSize overflows.
    ...
    inline auto FrameAllocator<N>::Deallocate(void* p) -> void
    {
        ...
        if (!OwnsPointer(p))
        {
            MAF_DEALLOC_FAIL("FrameAllocator does not support deallocating foreign pointers");
        }

        if (!IsActiveAllocationStart(p))
        {
            MAF_DEALLOC_FAIL("FrameAllocator Deallocate only accepts currently-live allocation start pointers");
        }
    }
    ...
    template<unsigned int N>
    inline auto FrameAllocator<N>::GetCapacity() const -> size_t
    {
        return static_cast<size_t>(N) * _frameSize;
    }

    template<unsigned int N>
    inline auto FrameAllocator<N>::GetUsedSpace() const -> size_t
    {
        size_t used = 0;
        for (const FrameState& frame : _frames)
        {
            if (frame.frameBegin && frame.current)
                used += static_cast<size_t>(frame.current - frame.frameBegin);
        }
        return used;
    }

    template<unsigned int N>
    inline auto FrameAllocator<N>::GetFreeSpace() const -> size_t
    {
