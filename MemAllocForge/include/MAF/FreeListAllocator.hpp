...
namespace MAF
{
    /// @brief First-fit free-list allocator with automatic coalescing.
    /// @details Maintains a sorted linked list of free blocks. Allocation performs first-fit
    ///          search (O(n)); deallocation coalesces with adjacent free neighbors (O(1)).
    ///          Supports arbitrary alignment. Not thread-safe.
    /// @param size Total arena size in bytes.
    /// @param defaultAlignment Default alignment (must be power of 2, >= alignof(max_align_t)).
    /// @throws std::invalid_argument If size is too small or defaultAlignment is not a power of 2.
    ...
    public:
        ...
        {
            size_t freeBytes = 0;
            LinkedNode* node = _pFirstNode;
            while (node)
            {
                if (!node->Used())
                    freeBytes += node->GetSize() + sizeof(LinkedNode);
                node = GetNextNode(node);
            }
            return _size - freeBytes;
        }
    ...
    private:
        ...
        {
            const uintptr_t userAddr = ToAddr(static_cast<const uint8_t*>(userPtr));
            const uintptr_t dataBeginAddr = ToAddr(_pData);
            const uintptr_t dataEndAddr = dataBeginAddr + _size;
            const uintptr_t minimumUserAddr = dataBeginAddr + sizeof(LinkedNode) + DISTANCE_STORAGE_SIZE;

            if (userAddr < minimumUserAddr || userAddr >= dataEndAddr)
            {
                MAF_DEALLOC_FAIL_VAL("FreeListAllocator pointer was not allocated by this allocator", nullptr);
            }
            ...
            if (!pHeader->Used())
            {
                MAF_DEALLOC_FAIL_VAL("FreeListAllocator pointer was already freed (double-free detected)", nullptr);
            }
        ...
        auto GetNextNode(LinkedNode* pNode) const -> LinkedNode*
        {
            ...
            const uintptr_t nodeAddr = ToAddr(pNode);
            const uintptr_t dataEndAddr = ToAddr(_pData) + _size;
            const size_t headerSize = sizeof(LinkedNode);
        ...
        auto IsValidHeader(const LinkedNode* pHeader) const -> bool
        {
            ...
            const uintptr_t dataBeginAddr = ToAddr(_pData);
            const uintptr_t dataEndAddr = dataBeginAddr + _size;
            const uintptr_t headerStartAddr = ToAddr(pHeader);
            const uintptr_t headerEndAddr = headerStartAddr + sizeof(LinkedNode);
            return headerStartAddr >= dataBeginAddr &&
                   headerEndAddr <= dataEndAddr &&
                   (headerStartAddr % NODE_ALIGNMENT) == 0;
        }

        auto FindNodeByAddress(uintptr_t headerAddr) const -> LinkedNode*
        {
            LinkedNode* pCurrentNode = _pFirstNode;
            while (pCurrentNode != nullptr)
            {
                const uintptr_t currentAddr = ToAddr(pCurrentNode);
