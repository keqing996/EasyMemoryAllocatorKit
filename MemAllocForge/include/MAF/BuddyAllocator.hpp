...
namespace MAF
{
    /// @brief Power-of-two buddy system allocator with O(log n) alloc/free.
    /// @details Arena size is rounded to a power of two. Blocks are split and merged in pairs.
    ///          Supports alignment up to the arena size. Not thread-safe.
    /// @param size Desired arena size (rounded up to next power of 2).
    /// @param minBlockSize Minimum block size (must be power of 2).
    /// @throws std::invalid_argument If minBlockSize is not a power of 2.
    ...
    inline auto BuddyAllocator::Deallocate(void* ptr) -> void
    {
        ...
        if (address < base)
        {
            MAF_DEALLOC_FAIL("BuddyAllocator pointer does not belong to this allocator");
        }

        const size_t offset = static_cast<size_t>(address - base);
        if (offset >= _size || (offset % MIN_BLOCK_SIZE) != 0)
        {
            MAF_DEALLOC_FAIL("BuddyAllocator pointer is not aligned to a block boundary");
        }

        const size_t index = offset / MIN_BLOCK_SIZE;
        if (_blockMeta[index].state != BlockState::UsedHead)
        {
            MAF_DEALLOC_FAIL("BuddyAllocator pointer is not an active allocation (possible double-free)");
        }
    ...
    inline auto BuddyAllocator::SetBlockState(size_t index, size_t order, BlockState headState) -> void
    {
        ...
        assert(index + span <= _minBlockCount);
    ...
    inline auto BuddyAllocator::DeallocateBlock(size_t index, size_t order) -> void
    {
        while (order + 1 < _maxOrder)
        {
            ...
            assert(buddyIndex < _minBlockCount);
    ...
    inline auto BuddyAllocator::GetUsedSpace() const -> size_t
    {
        size_t used = 0;
        for (size_t i = 0; i < _minBlockCount; )
        {
            if (_blockMeta[i].state == BlockState::UsedHead)
            {
                used += GetSizeFromOrder(_blockMeta[i].order);
                i += size_t(1) << _blockMeta[i].order;
            }
            else if (_blockMeta[i].state == BlockState::FreeHead)
            {
                i += size_t(1) << _blockMeta[i].order;
            }
            else
    ...
    inline auto BuddyAllocator::GetFreeSpace() const -> size_t
    {
