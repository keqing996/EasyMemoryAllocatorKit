...
namespace MAF
{
    /// @brief LIFO allocator with O(1) push/pop semantics.
    /// @details Each Allocate() pushes a frame; Deallocate() pops the most recent frame.
    ///          TryDeallocate(ptr) validates that ptr is the current stack top. Not thread-safe.
    ...
    inline auto StackAllocator::GetCapacity() const -> size_t
    {
        return _size;
    }

    inline auto StackAllocator::GetUsedSpace() const -> size_t
    {
        return GetFrameStartOffset();
    }

    inline auto StackAllocator::GetFreeSpace() const -> size_t
    {
        return _size - GetFrameStartOffset();
    }
}
