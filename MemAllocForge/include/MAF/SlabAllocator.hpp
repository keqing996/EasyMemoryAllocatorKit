...
namespace MAF
{
    /// @brief Dynamically-growing slab allocator for fixed-size objects.
    /// @details Allocates new slabs on demand, each containing a fixed number of same-sized slots.
    ///          O(1) alloc/free via a global free list across all slabs. Not thread-safe.
    /// @param objectSize Size of each object in bytes.
    /// @param objectsPerSlab Number of slots per slab (default 64).
    /// @param alignment Object alignment (must be power of 2, >= alignof(max_align_t)).
    /// @throws std::invalid_argument If objectSize is 0 or alignment is not a power of 2.
    class SlabAllocator
    {
    private:
        struct Slab
    ...
    public:
        ...
        size_t GetCapacity() const { return _slabCount * _slabDataSize; }
        size_t GetUsedSpace() const { return _allocationCount * _slotSize; }
        size_t GetFreeSpace() const { return GetCapacity() - GetUsedSpace(); }
...
#else
        TryLocatePointer(slot, slab, slotIndex);
#endif
    ...
    inline auto SlabAllocator::Deallocate(void* ptr) -> void
    {
        ...
        {
            MAF_DEALLOC_FAIL("SlabAllocator pointer does not belong to this allocator");
        }

        if (slab->slotStates[slotIndex] != SlotAllocated)
        {
            MAF_DEALLOC_FAIL("SlabAllocator pointer is not an active allocation (possible double-free)");
        }
