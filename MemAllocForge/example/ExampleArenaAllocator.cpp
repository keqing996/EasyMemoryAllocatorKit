...
int main()
{
    ...
    MAF::ArenaAllocator editorArena(4 * 1024 * 1024);
    ...
    std::vector<MAF::ArenaAllocator::Checkpoint> undoStack;
    ...
    printf("--- Session 5: Multi-Document Editing ---\n");
    
    auto checkpoint5 = editorArena.SaveCheckpoint();
    
    Document* docs[3];
    docs[0] = createDocument("// main.cpp\n#include <iostream>", 20);
    docs[1] = createDocument("# README.md\nProject documentation", 25);
    docs[2] = createDocument("{\n  \"config\": \"value\"\n}", 10);
    
    printf("Opened 3 documents in editor:\n");
    for (int i = 0; i < 3; ++i) {
        if (docs[i]) {
            printf("  Doc %d: %zu bytes, %d lines\n", 
                   i + 1, docs[i]->contentSize, docs[i]->lineCount);
        }
    }
...
T* New(Allocator& pAllocator)
{
    void* pMem = pAllocator.Allocate(sizeof(T), alignof(T));
    if (pMem == nullptr)
        return nullptr;
