...
TEST_CASE("ArenaAllocator - ScopeGuard checkpoint is invalidated by Reset")
{
    ArenaAllocator arena(1024, 8);
    ...
    CHECK(arena.GetUsedBytes() > 0);
    MESSAGE("ScopeGuard after Reset leaves bytes allocated because the checkpoint generation is stale");
}
