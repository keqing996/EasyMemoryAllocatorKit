...
TEST_CASE("StackAllocator - high-alignment padding preserves earlier data")
{
    StackAllocator allocator(8192, 8);
    constexpr std::uint8_t kMarkerA = 0xAB;
    constexpr std::uint8_t kMarkerB = 0xCD;

    void* a = allocator.Allocate(64, 1);
    REQUIRE(a != nullptr);
    std::memset(a, kMarkerA, 64);

    void* b = allocator.Allocate(32, 256);
    REQUIRE(b != nullptr);
    CHECK(IsAligned(b, 256));
    std::memset(b, kMarkerB, 32);

    for (size_t i = 0; i < 64; ++i)
        CHECK(static_cast<std::uint8_t*>(a)[i] == kMarkerA);

    for (size_t i = 0; i < 32; ++i)
        CHECK(static_cast<std::uint8_t*>(b)[i] == kMarkerB);

    allocator.Deallocate();
    CHECK(allocator.GetStackTop() == a);
    allocator.Deallocate();
    CHECK(allocator.GetStackTop() == nullptr);
}

TEST_CASE("StackAllocator - interleaved pop and reallocation reuses space correctly")
{
    ...
    REQUIRE(c != nullptr);
    CHECK(IsAligned(c, 16));
    CHECK(allocator.GetStackTop() == c);
...
TEST_CASE("StackAllocator - TryDeallocate on an empty stack returns false")
{
...
TEST_CASE("StackAllocator - dense one-byte allocations fill and unwind the buffer")
{
    constexpr size_t kBufferSize = 512;
    StackAllocator allocator(kBufferSize, 8);

    std::vector<void*> ptrs;
    while (true)
    {
        void* p = allocator.Allocate(1, 1);
        if (!p)
            break;
        ptrs.push_back(p);
    }
    ...
    {
        CHECK(allocator.GetStackTop() == *it);
        allocator.Deallocate();
    }
    CHECK(allocator.GetStackTop() == nullptr);
}
