...
| `ENABLE_MAF_SANITIZERS` | Build MemAllocForge targets with ASan + UBSan (Clang/GCC, default `OFF`) |

Sanitizers are opt-in and apply to MemAllocForge targets generated by this project:

```cmake
cmake -S . -B build-asan -DENABLE_MAF_TEST=ON -DENABLE_MAF_SANITIZERS=ON
...
The test suite includes both strict-mode tests and a dedicated lenient-mode target to lock down the expected no-op behavior when `MAF_DEALLOCATE_STRICT=0`.
...
These counters are allocator-specific accounting values. For segmented or circular allocators, `GetFreeSpace()` is not always the same thing as "largest payload that can be allocated next"; use allocator-specific helpers such as `GetCurrentFrameAvailableSpace()`, `GetLargestFreeContiguousSpace()`, or `CanAllocate()` when that distinction matters.
...
| Property | Value |
|---|---|
| Allocate | O(1) bitmap-assisted search plus bounded list scan |
| Deallocate | O(1) with coalescing |
| Thread-safe | No |
| Use case | Real-time systems, game engines, latency-sensitive workloads |

Two-Level Segregated Fit with bounded fragmentation. Bitmaps narrow the search to candidate bins, while this implementation may still scan within a bin's free list to find a block that satisfies size and alignment. Template parameters `FL_COUNT` and `SL_COUNT` control granularity.
...
Lifecycle contract: all threads using a `ThreadCachingAllocator` must stop before the allocator is destroyed. Pointers returned by the allocator become invalid at destruction, and calling `Deallocate()` after allocator destruction is undefined behavior.
